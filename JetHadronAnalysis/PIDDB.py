...
def getParticleFractionForMomentumBin(analysisType, region, momentum_bin, particleType, dbCursor):
    ...
    # define the SQL query
    query = f"SELECT {particleType.name.lower()}_fraction, {particleType.name.lower()}_fraction_error FROM particle_fractions WHERE analysis_type = ? AND region = ? AND momentum_bin = ?;"
    # execute the query
