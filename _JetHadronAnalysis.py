...
class AnalysisMixin:
    ...
    def get_SE_correlation_function_for_species(self, species):
        ...
        speciesID = 8 if species == "pion" else 9 if species == "proton" else 10 if species == "kaon" else 11
        TH2Corr = None
        for sparse_ind in range(len(self.JH)): #type:ignore
            self.JH[sparse_ind].GetAxis(speciesID-offset).SetRangeUser(-2,2) #type:ignore
            if TH2Corr is None:
                TH2Corr = self.JH[sparse_ind].Projection(4,3) #type:ignore
                TH2Corr.Sumw2()
            else:
                TH2Corr.Add(self.JH[sparse_ind].Projection(4,3)) #type:ignore
            self.JH[sparse_ind].GetAxis(speciesID-offset).SetRangeUser(-10,10) #type:ignore
        return TH2Corr.Clone()
    ...
    def get_SE_correlation_function_w_pionTPCnSigma(self, species):
        ...
        '''
        offset = 1 if self.analysisType=="pp" else 0 #type:ignore
        if self.get_SE_correlation_function_w_Pion_has_changed[species] or self.get_SE_correlation_function_w_Pion_result[species] is None: #type:ignore
            speciesID = 8 if species == "pion" else 9 if species == "proton" else 10 if species == "kaon" else 11
            ...
            for sparse_ind in range(len(self.JH)): #type:ignore
                self.JH[sparse_ind].GetAxis(speciesID-offset).SetRangeUser(-2,2) #type:ignore
                if TH3Corr is None:
                    TH3Corr = self.JH[sparse_ind].Projection(3,4,7-offset) #type:ignore
                    TH3Corr.Sumw2()
                else:
                    TH3Corr.Add(self.JH[sparse_ind].Projection(3,4,7-offset)) #type:ignore
                self.JH[sparse_ind].GetAxis(speciesID-offset).SetRangeUser(-10,10) #type:ignore
            self.get_SE_correlation_function_w_Pion_result[species] = TH3Corr #type:ignore
            self.get_SE_correlation_function_w_Pion_has_changed[species] = False #type:ignore
            return TH3Corr.Clone()
        else:
            return self.get_SE_correlation_function_w_Pion_result[species].Clone() #type:ignore
    ...
    def get_normalized_ME_correlation_function(self):
        if self.get_normalized_ME_correlation_function_has_changed or self.get_normalized_ME_correlation_function_result is None:
            ...
            return self.get_normalized_ME_correlation_function_result.Clone(), self.get_ME_normalization_error
    ...
    def get_acceptance_corrected_correlation_function_w_pionTPCnSigma(self, species):
        if self.get_acceptance_corrected_correlation_function_w_pionTPCnSigma_has_changed[species] or self.get_acceptance_corrected_correlation_function_w_pionTPCnSigma_result[species] is None:
            # get the raw correlation functions
            ...
            NormMEC_3d = TH3D("NormMEC_3d", "NormMEC_3d", xbins, xlow, xhigh, ybins, ylow, yhigh, zbins, zlow, zhigh)
            ...
            for i in range(0, xbins+2):
                for j in range(0, ybins+2):
                    for k in range(0, zbins+2):
                        NormMEC_3d.SetBinContent(i, j, k, NormMEC.GetBinContent(i, j)/zbins) # yay for github copilot ! 🥳🥳🥂
                        NormMEC_3d.SetBinError(i, j, k, NormMEC.GetBinError(i, j)/zbins)

            # divide the raw correlation function by the mixed event correlation function
            Corr.Divide(NormMEC_3d) 
            #Corr.GetXaxis().SetRangeUser(-0.9, 0.9)
            del NormMEC
            self.get_acceptance_corrected_correlation_function_w_pionTPCnSigma_result[species]= Corr
            self.get_acceptance_corrected_correlation_function_w_pionTPCnSigma_has_changed[species] = False
            return Corr.Clone()
        else:
            return self.get_acceptance_corrected_correlation_function_w_pionTPCnSigma_result[species].Clone()
    ...
    def get_N_trig(self):
        ...
        for sparse_ind in range(len(self.Trigger)):
            if N_trig_hist is None:
                N_trig_hist = self.Trigger[sparse_ind].Projection(1) # type:ignore the projection wont count the entries that are outside of the SetRangeUser, whereas the THnSparse will  
            else:
                N_trig_hist.Add(self.Trigger[sparse_ind].Projection(1)) # type:ignore the projection wont count the entries that are outside of the SetRangeUser, whereas the THnSparse will
    ...
    def get_dPhi_projection_in_dEta_dpionTPCnSigma_range(self, dEtaRange, TOFcutSpecies, rebin=True, scaleUp=False):
        '''
        Returns the dPhi distribution for a given dEta range
        '''
        tempHist = self.get_acceptance_corrected_correlation_function_for_species(TOFcutSpecies)
        lowerBin, upperBin= (tempHist.GetXaxis().FindBin(bin) for bin in dEtaRange)
        tempHist.GetXaxis().SetRange(lowerBin, upperBin)
        dPhi = tempHist.ProjectionY()
        dPhi.Sumw2()
    ...
    def get_dPhi_dpionTPCnSigma_projection_in_dEta_range(self, dEtaRange, TOFcutSpecies, rebin=True, scaleUp=False):
        '''
        Returns the dPhi distribution for a given dEta range
        '''
        tempHist = self.get_acceptance_corrected_correlation_function_w_pionTPCnSigma(TOFcutSpecies)
        lowerBin, upperBin= (tempHist.GetXaxis().FindBin(bin) for bin in dEtaRange)
        tempHist.GetXaxis().SetRange(lowerBin, upperBin)
    ...
    def get_normalized_BG_subtracted_AccCorrectedSEdPhiSig_for_species(self, i,j,k, species):
        '''
        ...
        dPhiSigInclusiveBinContents = self.get_bin_contents_as_array(dPhiSigInclusive, forFitting=False) # type:ignore
        # get the bin errors of the inclusive dPhi distribution
        dPhiSigInclusiveBinErrors = self.get_bin_errors_as_array(dPhiSigInclusive, forFitting=False) # type:ignore
        # get the bin centers of the inclusive dPhi distribution
        x_valsInclusive = self.get_bin_centers_as_array(dPhiSigInclusive, forFitting=False) # type:ignore
        if self.analysisType in ["central", "semicentral"]: # type:ignore
            # generate the background distribution from the RPFObj 
            ...
            # 
            # subtract the background from the signal 
            BGSubtracteddPhiSig = dPhiSigBinContents- dPhiBGRPF*(dPhiSigBinContents/dPhiSigInclusiveBinContents)
            # get the error on the background subtracted signal
            BGSubtracteddPhiSigErr = np.sqrt(dPhiSigBinErrors**2)
            ...
            # get the minimum value of the signal
            minVal, minValErr = self.get_minVal_and_systematic_error(dPhiSigBinContents)
            minVal = self.NormalizedBGSubtractedAccCorrectedSEdPhiSigcorrsminVals[i,j] #type:ignore
            # subtract the minimum value from the signal
            BGSubtracteddPhiSig = dPhiSigBinContents - minVal
            # get the error on the background subtracted signal
            BGSubtracteddPhiSigErr = np.sqrt(dPhiSigBinErrors**2)
    ...
    def get_normalized_background_subtracted_dPhi_for_species(self, i,j,k, species):
        ...
        if self.analysisType in ['central', 'semicentral']: # type:ignore
            ...
            BGSubtracteddPhiSigForSpeciesHist = self.get_normalized_BG_subtracted_AccCorrectedSEdPhiSig_for_species(i,j,k, species)
            ...
            # get the background subtracted dPhi distribution for a given species
            BGSubtracteddPhiSigForSpeciesHist, minValErr = self.get_normalized_BG_subtracted_AccCorrectedSEdPhiSig_for_species(i,j,k, species)
            # return the background subtracted dPhi distribution
    ...
    def get_yield_for_species(self, i,j,k, species, region=None):
        ...
        if self.analysisType in ['central', 'semicentral']: # type:ignore
            ...
            BGSubtracteddPhiSigdpionTPCnSigmaHist = self.get_normalized_background_subtracted_dPhi_for_species(i,j,k, species)
        elif self.analysisType == "pp": # type:ignore
            # get the normalized background subtracted dPhi distribution for a given species
            BGSubtracteddPhiSigdpionTPCnSigmaHist, minValErr = self.get_normalized_background_subtracted_dPhi_for_species(i,j,k, species) # type:ignore
        # get the yield for a given species
        bin_width = BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinWidth(1)
        if region is None:
            yield_ = np.sum(BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinContent(l)*bin_width for l in range(1, BGSubtracteddPhiSigdpionTPCnSigmaHist.GetNbinsX()+1))
            yield_err_ = np.sqrt(np.sum((BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinError(l)*bin_width)**2 for l in range(1, BGSubtracteddPhiSigdpionTPCnSigmaHist.GetNbinsX()+1)))
        elif region=='NS':
            # get bins at location dPhiSigNS[0] and dPhiSigNS[1]
            low_bin, high_bin = BGSubtracteddPhiSigdpionTPCnSigmaHist.GetXaxis().FindBin(self.dPhiSigNS[0]), BGSubtracteddPhiSigdpionTPCnSigmaHist.GetXaxis().FindBin(self.dPhiSigNS[1])
            yield_ = np.sum(BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinContent(l)*bin_width for l in range(low_bin, high_bin+1))
            yield_err_ = np.sqrt(np.sum((BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinError(l)*bin_width)**2 for l in range(low_bin, high_bin+1)))
        elif region=='AS':
            low_bin, high_bin = BGSubtracteddPhiSigdpionTPCnSigmaHist.GetXaxis().FindBin(self.dPhiSigAS[0]), BGSubtracteddPhiSigdpionTPCnSigmaHist.GetXaxis().FindBin(self.dPhiSigAS[1])
            yield_ = np.sum(BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinContent(l)*bin_width for l in range(low_bin, high_bin+1))
            yield_err_ = np.sqrt(np.sum((BGSubtracteddPhiSigdpionTPCnSigmaHist.GetBinError(l)*bin_width)**2 for l in range(low_bin, high_bin+1)))
            # return the yield
        ...
        for sparse_ind in range(len(self.JH)): # type:ignore
            if region is not None:
                if region == 'signalNS':
                    ...
                    self.JH[sparse_ind].GetAxis(3).SetRangeUser(self.dEtaSig[0], self.dEtaSig[1]) #deta
                    self.JH[sparse_ind].GetAxis(4).SetRangeUser(self.dPhiSigNS[0], self.dPhiSigNS[1]) #dphi


                elif region == 'signalAS':
                    # restrict ourselves to the signal region
                    self.JH[sparse_ind].GetAxis(3).SetRangeUser(self.dEtaSigAS[0], self.dEtaSigAS[1])
                    self.JH[sparse_ind].GetAxis(4).SetRangeUser(self.dPhiSigAS[0], self.dPhiSigAS[1])
                if region == 'background':
                    pass
            if particleType == "pion":
                self.JH[sparse_ind].GetAxis(8-offset).SetRangeUser(-2,2) # type:ignore
                if pion_TPC_nSigma is None:
                    pion_TPC_nSigma = self.JH[sparse_ind].Projection(7-offset) # type:ignore
                    pion_TPC_nSigma.Sumw2()
                else:
                    pion_TPC_nSigma.Add(self.JH[sparse_ind].Projection(7-offset)) # type:ignore
                self.JH[sparse_ind].GetAxis(8-offset).SetRangeUser(-100,100) # type:ignore
            elif particleType == "proton":
                self.JH[sparse_ind].GetAxis(9-offset).SetRangeUser(-2,2) #type:ignore
                if pion_TPC_nSigma is None:
                    pion_TPC_nSigma = self.JH[sparse_ind].Projection(7-offset) #type:ignore
                    pion_TPC_nSigma.Sumw2()
                else:
                    pion_TPC_nSigma.Add(self.JH[sparse_ind].Projection(7-offset))
                self.JH[sparse_ind].GetAxis(9-offset).SetRangeUser(-100,100) #type:ignore
            elif particleType == "kaon":
                self.JH[sparse_ind].GetAxis(10-offset).SetRangeUser(-2,2) #type:ignore
                if pion_TPC_nSigma is None:
                    pion_TPC_nSigma = self.JH[sparse_ind].Projection(7-offset) #type:ignore
                    pion_TPC_nSigma.Sumw2()
                else:
                    pion_TPC_nSigma.Add(self.JH[sparse_ind].Projection(7-offset))
                self.JH[sparse_ind].GetAxis(10-offset).SetRangeUser(-100,100) #type:ignore
            elif particleType == "electron":
                self.JH[sparse_ind].GetAxis(11-offset).SetRangeUser(-2,2) #type:ignore
                if pion_TPC_nSigma is None:
                    pion_TPC_nSigma = self.JH[sparse_ind].Projection(7-offset) #type:ignore
                    pion_TPC_nSigma.Sumw2()
                else:
                    pion_TPC_nSigma.Add(self.JH[sparse_ind].Projection(7-offset))
                self.JH[sparse_ind].GetAxis(11-offset).SetRangeUser(-100,100) #type:ignore
            self.JH[sparse_ind].GetAxis(3).SetRange(0,self.JH[sparse_ind].GetAxis(3).GetNbins()+1) #deta # type:ignore
            self.JH[sparse_ind].GetAxis(4).SetRange(0,self.JH[sparse_ind].GetAxis(4).GetNbins()+1) #dphi # type:ignore
