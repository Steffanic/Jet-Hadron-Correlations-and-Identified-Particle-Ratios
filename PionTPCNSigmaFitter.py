...
class PionTPCNSigmaFitter:
    def __init__(self, p0=None, p0_bounds=None, w_inclusive=True, generalized=False):
        self.p0 = p0
        self.p0_bounds = p0_bounds
        self.popt = None
        self.pcov = None
        if generalized and not w_inclusive:
            raise NotImplementedError("Generalized fit only implemented without inclusive fitting")
        if w_inclusive:
            self.fit_func = self.piKpInc_enhanced_fit
            if generalized:
                self.fit_func = self.piKpInc_generalized_fit
        else:
            self.fit_func = self.piKp_enhanced_fit
    ...
    def ugauss(self, x, mu, sig, a):
        return a*unp.exp(-0.5*((x-mu)/sig)**2)
    
    def ugeneralized_gauss(self, x, mu, sig, a, alpha):
    ...
    def two_generalized_gaussians_and_one_gaussian(self, x, mup, mupi, muk, sigp, sigpi, sigk, ap, api, ak, alphap, alphak):
    ...
    def utwo_generalized_gaussians_and_one_gaussian(self, x, mup, mupi, muk, sigp, sigpi, sigk, ap, api, ak, alphap, alphak):
    ...
    def piKpInc_enhanced_fit(self, x, mup, mupi, muk, sigp, sigpi, sigk, app, apip, akp, appi, apipi, akpi, apk, apik, akk, apinc, apiinc, akinc):
        '''
        Returns an hstack of the three gaussians for each enhanced sample: pi, K, and p, and the inclusive. 
        x: [array] nsigma values
        mu*: [float] mean of the gaussian
        sig*:  [float] sigma of the gaussian
        a_{@,#}: [float] amplitude of the gaussian for the @ particle in the # enhanced sample
        ...
        if non_zero_masks is not None:
            return np.hstack([self.three_gaussians(x[non_zero_masks[0]], mup, mupi, muk, sigp, sigpi, sigk, appi, apipi, akpi), self.three_gaussians(x[non_zero_masks[1]], mup, mupi, muk, sigp, sigpi, sigk, app, apip, akp), self.three_gaussians(x[non_zero_masks[2]], mup, mupi, muk, sigp, sigpi, sigk, apk, apik, akk), self.three_gaussians(x[non_zero_masks[3]], mup, mupi, muk, sigp, sigpi, sigk, apinc, apiinc, akinc)])
        else:
            return np.hstack([self.three_gaussians(x, mup, mupi, muk, sigp, sigpi, sigk, appi, apipi, akpi), self.three_gaussians(x, mup, mupi, muk, sigp, sigpi, sigk, app, apip, akp), self.three_gaussians(x, mup, mupi, muk, sigp, sigpi, sigk, apk, apik, akk), self.three_gaussians(x, mup, mupi, muk, sigp, sigpi, sigk, apinc, apiinc, akinc)])
    
    def piKpInc_enhanced_error(self, x, mup, mupi, muk, sigp, sigpi, sigk, app, apip, akp, appi, apipi, akpi, apk, apik, akk, apinc, apiinc, akinc, pcov):
        partial_F_app = np.exp(-(x - mup)**2 / (2 * sigp**2))
        partial_F_appi = np.exp(-(x - mup)**2 / (2 * sigp**2))
        partial_F_apk = np.exp(-(x - mup)**2 / (2 * sigp**2))
        partial_F_apinc = np.exp(-(x - mup)**2 / (2 * sigp**2))
        partial_F_apip = np.exp(-(x - mupi)**2 / (2 * sigpi**2))
        partial_F_apipi = np.exp(-(x - mupi)**2 / (2 * sigpi**2))
        partial_F_apik = np.exp(-(x - mupi)**2 / (2 * sigpi**2))
        ...
        partial_F_akpi = np.exp(-(x - muk)**2 / (2 * sigk**2))
        partial_F_akk = np.exp(-(x - muk)**2 / (2 * sigk**2))
        partial_F_akinc = np.exp(-(x - muk)**2 / (2 * sigk**2))

        partial_F_mup = (app+appi+apk + apinc) * (x - mup) * np.exp(-(x - mup)**2 / (2 * sigp**2)) / sigp**2
        partial_F_mupi = (apip + apipi + apik + apiinc) * (x - mupi) * np.exp(-(x - mupi)**2 / (2 * sigpi**2)) / sigpi**2
        partial_F_muk = (akp + akpi + akk + akinc) * (x - muk) * np.exp(-(x - muk)**2 / (2 * sigk**2)) / sigk**2

        partial_F_sigp =  (app+appi+apk + apinc) * (x - mup)**2 * np.exp(-(x - mup)**2 / (2 * sigp**2)) / sigp**3
        partial_F_sigpi = (apip + apipi + apik + apiinc) * (x - mupi)**2 * np.exp(-(x - mupi)**2 / (2 * sigpi**2)) / sigpi**3
        partial_F_sigk = (akp + akpi + akk + akinc) * (x - muk)**2 * np.exp(-(x - muk)**2 / (2 * sigk**2)) / sigk**3
    ...
    def fit(self, x, y, yerr):
        ...
        #x_non_zero_mask = np.sum(non_zero_masks, axis=0) == len(y)
        xs = [x[non_zero_masks[i]] for i in range(len(y))]
        y = [y[i][non_zero_masks[i]] for i in range(len(y))]
        yerr = [yerr[i][non_zero_masks[i]] for i in range(len(yerr))]
        self.non_zero_masks = non_zero_masks
        popt_all, pcov_all = curve_fit(self.fit_func,x, np.hstack(y), p0=self.p0, bounds=self.p0_bounds, sigma=np.hstack(yerr), absolute_sigma=True , maxfev=1000000, jac=self.piKpInc_generalized_jac)
        self.popt = popt_all
        self.pcov = pcov_all

        NDF = len(np.hstack(y)) - len(self.popt) - 1
        chi2=np.sum(((np.hstack(y) - self.fit_func(x, *self.popt))**2)/(np.hstack(yerr))**2)
        
        self.chi2OverNDF = chi2/NDF
        self.non_zero_masks = None
